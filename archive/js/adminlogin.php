...
// Make sure the Fr\LS class is loaded
require_once $_SERVER['DOCUMENT_ROOT'] . "/Fr/LS.php"; // <-- Make sure this path matches the actual location of LS.php on your server

// Check if the class exists, and handle error if not
if (!class_exists('Fr\LS')) {
    // Try requiring with namespace fallback if LS.php does not declare the namespace
    require_once $_SERVER['DOCUMENT_ROOT'] . "/Fr/LS.php";
    if (!class_exists('Fr\LS')) {
        // Try requiring without namespace if LS.php does not declare the namespace
        require_once $_SERVER['DOCUMENT_ROOT'] . "/Fr/LS.php";
        if (!class_exists('LS')) {
            die("Error: Fr\\LS or LS class not found. Please check the namespace and path to LS.php.");
        } else {
            class_alias('LS', 'Fr\LS');
        }
    }
}
...
if($MyDIOlogin->loggedIn)
  {
    ...
    if(method_exists('\\Fr\\LS', 'isLoggedIn'))
		{

        //echo "COOL".(\Fr\LS::isLoggedIn() ? '1' : '0')."w"; exit();
			}
		else
			{	
	...
	//echo "COOL".\Fr\LS::$loggedIn."s"; exit();
                ...
                //echo "COOL".(\Fr\LS::isLoggedIn() ? '1' : '0')."s"; exit();
							{
                //echo " <p>" . (method_exists('\Fr\LS', 'isLoggedIn') && \Fr\LS::isLoggedIn() ? '1' : '0');
								$login = false;//\Fr\LS::login($identification, $password, $remember );
								if($login === false)
									{
