...
if($_GET['gfn_eod'])
	{
		$gfn=true;
		$_SESSION['gfn_eod']=true;	
		$veg=$_GET['veg_pledge'];
		$pap=$_GET['paper_pledge'];
		if($veg=='true'||$_SESSION['eod_veg'])
		{
			$_SESSION['eod_veg']=true;
			$rews=$rews+1;
			$show_message='';
			
		}
		if($pap=='true'||$_SESSION['eod_paper'])
		{
			$_SESSION['eod_paper']=true;	
			$rews=$rews+1;
			$show_message='';
		...
		$congrat_message="Your pledge".$plur." earn".$plurs." you ".$final_rew." DIO credits<br><p>&nbsp;</p>Use DIO, through our exclusive Earth Overshoot Day partnership, when you buy a Ticket to the Moon Hammock";
	}
else
	{
		$gfn=false;
		unset($_SESSION['gfn_eod']); 
	}
include("../../../login/loggedin/get-profile/get-profile.php");
$company_data=unserialize($_POST['comp_dets']);
		...
		if($row = mysqli_fetch_array($result, MYSQLI_BOTH))
				...
				{ 
					if(!empty($_POST['city']))
					...
					else
						{
							...
							$city=\MyDIO_Functions\Functions_MyDIO::getCity($city);
						}
				}
		...
		else
			{
				...
				$logo		=$_POST['logo'];
				$city		=$_POST['city'];
				$cmpydescon	=$_POST['cmpydescon'].
				$ydio		=$_POST['ydio'];
				$wwwsite	=$_POST['wwwsite'];
				$condits	=$_POST['condits'];
				$map		=$_POST['map'];
				...
				$ecomm_status=$_POST['ecomm_active'];
				$market_data			=$row['market'];
			}
		 ...
		 if($_GET['app'])
 {
	 ...
	 if($_GET['checkin'])
	 {
		$checkedin=$_GET['checkin'];
	 }

  $market_data= unserialize($market_data);
							  //print_r($market_data);
...
<?
						...
						if($city!="Global")
						{
							include("../../../resources/map/nearest_business.php");
						}
...
<html>
		...
		<? if($collaborators||($map=='address found' && $sub_industry!="private_chauffeured_transport"))
  		{ ?>
   ...
   if($map=='address found' && $sub_industry!="private_chauffeured_transport")
  		{ ?>
		  ...
		  { 
						...
						google.maps.event.addListener(myMarker, 'dragend', function(evt)
							{
								...
								getnearest(evt.latLng.lat().toFixed(5), evt.latLng.lng().toFixed(5));
							});
			...
			// init map
				map = new google.maps.Map(document.getElementById('map_frame'), options); 
				 google.maps.event.trigger(map, 'resize'); 
function getnearest(tolat,tolng)
			{
				...
				var fromlng;
				 ...
				 $.ajax({
							...
							$.each(data, function(){
											//Plot the location as a marker		
									  var pos = new google.maps.LatLng(this.lat, this.lng);	
									  // calculate nearest driver(s) to pickup point
									  drivlat = this.lat;
										drivlng = this.lng;
										var	driver_name=(this.driver_fullname);
										...
										if(drivlat>0)
											{
												...
												var mlat =tolat;
												var mlng = tolng;
												var dLat  = rad(mlat - drivlat);
												var dLong = rad(mlng - drivlng);
												var a = Math.sin(dLat/2) * Math.sin(dLat/2) +
													Math.cos(rad(drivlat)) * Math.cos(rad(drivlat)) * Math.sin(dLong/2) * Math.sin(dLong/2);
												...
												if ( closest == -1 || d < distances[closest] )
													{
														...
														closest = i;
														closest_driver_name=(this.driver_fullname);
														distance=d;
														id= (this.driver_id);
														fromlat=drivlat;
														fromlng=drivlng;
									...
									// Finish calculating nearest driver
								   ...
								   (function(marker)
												{
													...
													if(online==false)
														...
														{
																	infowindow = new google.maps.InfoWindow({
																			content: "<div style=\"color: #000; text-align: left;\" ><b>" + driver_name +"</a></b><p>"+parseFloat(d).toFixed(2) + " Km</div>"
																		});
														}
													 if (infowindow)
														{
									...
									time_cal(fromlat,fromlng,tolat,tolng);
									//alert('from: '+fromlat+','+fromlng+' to:'+tolat+','+tolng);
													document.getElementById("nearest-driver").innerHTML = 'Nearest driver '+ parseFloat(distance).toFixed(2) + ' Km';
			...
			function time_cal(fromlat,fromlng,tolat,tolng){
			var durat='';
		var distanceService = new google.maps.DistanceMatrixService();
		//alert(fromlat+','+fromlng);
    distanceService.getDistanceMatrix({
	 ...
	 function (response, status) {
        if (status !== google.maps.DistanceMatrixStatus.OK)
		{
			...
			durat=response.rows[0].elements[0].duration.text;
			           // alert(durat);
			document.getElementById("waiting").innerHTML = 'wait: ' + durat;
        }
    });
...
if($_GET['app'])
{ ?>
		<body leftmargin="3px" marginwidth="3px" class="no-sidebar">
<? } else
{ ?>
<body leftmargin="3px" marginwidth="3px" class="no-sidebar header-detached">
<? } ?>
<div id="fb-root"></div>
<!--[if lt IE 8]>
...
<![endif]-->
<noscript>
        <div class="alert alert-error alert-top"> It looks like your browser doesn't support javascript. This page will work a lot better for you if you activate javascript or download a newer browser like <a href="https://www.google.com/chrome/" target="_blank">Google Chrome</a> </div>
        </noscript>
<?
if($_GET['app'])
{ 
...
if($logged)
   ...
   ?>
<div class="header">
          <nav> <a  class="hamburger"
                data-toggle="dropdown"
                data-func="LoadFromTopBar"
                href="/login/loggedin/account.php">About</a>
    <li> · </li>
...
<script>
  ...
  jQuery(function($){
		$("a.close_prompt").click(function(){
		document.getElementById('rew_msg').style.display="none";
		});
		});	
		</script>
<div class="busiframe">
    ...
    <div class="fade_in">
              <div id="rew_msg" class="infobox" style="display:none;">
        <div class="postcode">
                  <div style="width:20px; position:absolute; height:20px; top:5; text-align:right; left:227px;"><a class="close_prompt">
                    <button type="button" name="fade" class="btn btn-flat btn-primary btn-large">X</button>
                    </a></div>
                  <h6>&nbsp;</h6>
                  <p align="center" id="msg_img" > </p>
                  <h6>&nbsp;</h6>
                  <h3>Congratulations</h3>
                  <h4 id="msg_txt"><? echo $congrat_message; ?></h4>
                  <h6>&nbsp;</h6>
                </div>
      </div>
...
?>
        ...
        <tr> <?php echo "<a href=//$wwwsite>" ?>
                  <td class="size" height="180" rowspan="2" background="../../../businesssignup/<?php echo $img; ?>"><p>&nbsp;</p></td>
                  <?php echo "</a>" ?> </tr>
      </table>
              <div class="<? if($rews>0){ echo "about_frame"; }else{ echo "busyprofile"; } ?>">
        <? if($rews>0){  ?>
        <div  <? if($rews>0){ echo "style='display:inline-block;'"; }else{ } ?>>
        ...
        <div  <? if($rews>0){ echo "style='display:inline-block;'"; }else{ } ?>>
            ...
            <div class="logonav">
                      <div class="buslogo"  style="background-image: url('../../../businesssignup/<?php echo $logo; ?>');"></div>
                    </div>
            <table align="left" width="300" >
                      <tr valign="top">
                <td width="77" height="46" align="left" padding="3">&nbsp;</td>
                <td width="203" align="left" padding="3"><h5><?php echo $companyname ?></h5>
                          <h6>
                    <?php 
	   ...
	   echo $city ?>
                      ...
                      <tr valign="top">
                <td height="60" padding="3" align="left" colspan="2"><p>
                  
                          <h6><?php echo $cmpydescon ?></h6></td>
              </tr>
                    </table>
            <script>
								jQuery(function($){
 ...
 if($drivers)
	{ ?>
      ...
      <img src="/images/icon_MyDIO.png" width="40" height="40">
		...
		if($ecomm_status=='TRUE' OR $mydio_store)
			...
			?>
              ...
              <table height="50px">
                        <tr>
                  <td><h4>Pay <?php echo $status; ?>% here with DIO<br>
                    </h4></td>
                  <td><img src="/images/icon_MyDIO.png" width="40" height="40"></td>
                </tr>
                      </table>
              </button>
                    </form>
        ...
        <? if($drivers)
	 	{ ?>
        <div class="reveal_map">
                  <div id="map_frame" class="small_map_cadre"></div>
                </div>
        <p>&nbsp;</p>
        <div><a class="user-location">
          <button class="btn btn-flat btn-action btn-primary btn-large btn-large-green">Order a Taxi-Bike</button>
          </a>
                  <p>&nbsp;</p>
                </div>
        <? } ?>
		...
		if (mysqli_query($bd,$sql)) 
			{
				...
				if ($rows > '0')
					{
						echo "<h4><br><p>Rewardable actions at ".$companyname."<br></h4>";
						...
						while ($output = mysqli_fetch_assoc($result))
							{		
								...
								if($output['active'])
								{
								$sql2 = "SELECT * FROM community_actions WHERE action_id='$act_id'";
								if (mysqli_query($bd,$sql)) 
									{
									...
									{
										echo "Error: " . $sql2 . "<br>" . mysqli_error($bd);
									}
							
								$rows2 = mysqli_num_rows($result2);
								if ($rows2 > '0')
									{  
								   		while ( $output2 = mysqli_fetch_assoc($result2))
											{
												echo "<div class='reward_profile' id='fullprofile3'><div class=\"rewardable_actions\">";
												echo "<div class=\"padding_actions\">";
												//echo "<div class=\"profile_name\">".$output['action_name']."</div>";
												if (!empty($output2['action_image']))
													{
														echo "<div class=\"profile_image\" style=\"background: url('".$output2['action_image']."'); background-size: cover; height:150px;\"></div>";
													}
												//echo "<div class=\"profile\">";
												echo "<div class=\"whitebackground\">";
												echo "<div>";
												echo "<h4><a href='/community/action.php?redirect=".$redirect."&id=".$output2['community_id']."&action=".$output2['action_id']."'>".$output2['action_name']."</a></h4>";
												if (!empty($output2['action_description']))
													{
														echo " <table align='left' width='300' >
														<tr valign='top'>
														  <td  align='left' padding='3'><h6>".substr($output2['action_description'], 0, '210') . "</h6><a href='/community/action.php?id=".$output2['community_id']."&action=".$output2['action_id']."'> - Read More...</a>";
														echo "<br></td></tr></table>";
													}
												//echo "<div style=\"text-align:center;\"><a class=\"btn\" href='action.php?id=".$output['community_id']."&action=".$output['action_id']."'>Earn ".$output['dio']." DIO</a></div>";
													echo "<div style=\"text-align:center;\"><a href='/community/action.php?redirect=".$redirect."&id=".$output2['community_id']."&action=".$output2['action_id'].$append."'><button class=\"btn btn-flat btn-action btn-primary btn-large\"> Earn "; if($output2['percentage_invoice']){ echo ($output['dio_contribution']+0)."% of the value of your "; if($output2['purchase']=="Ticket"){echo "ticket"; }else{echo "invoice"; }; echo "  in DIO";}else{echo ($output['dio_contribution']+$output2['dio']+0)." DIO";}; echo "</button></a></div>";
												echo "</div>";
												//echo "</div>";
												echo "<br>";	
												echo "</div>";
				...
				?>
              <p>
      <div class="fullprofile tttm_web" id="fullprofile4">
        <? if(!$address1==''){ ?>
        <table align="left" width="300" >
          <tr valign="top">
	 ...
	 { ?>
        ...
        <table align="left" width="300" >
                  <tr valign="top">
            <td  align="left" padding="3"><h5>Why we accept DIO:</h5></td>
          </tr>
                  <tr valign="top">
            <td height="60" padding="3" align="left" colspan="2"><p>
        ...
        <p> 
              ...
              <? } if($ecomm_status=='TRUE'&&!$gfn)
			{ ?>
              <div class="fullprofile" id="fullprofile8">
        <table align="left" width="300" >
                  <tr valign="top">
            <td height="30" padding="3" align="left" colspan="2"><p>
              
                      <h6>
                <form action="/MyDIOnlineStore/product_list.php" target="_blank">
                          <input type="hidden" name="company_id" value='<? echo $companyid; ?>' >
                          <input type="hidden" name="company_name" value='<? echo $companyname; ?>' >
              ...
              <? } ?>
              <? if(! $wwwsite==''){ ?>
              <div class="fullprofile" id="fullprofile6"  <? if($gfn){ echo "style='width:100%'";} ?> >
        <table id="fullprofile9" align="left" width="300" >
                  <tr valign="top">
            <td height="30" padding="3" align="left" colspan="2"><p>
              
                      <h6>
                <form action="<?php echo "//".$wwwsite ?>" target="_blank" method="get">
                          <button type="submit" class="btn btn-flat-black btn-primary btn-large-black" <? if($gfn){ echo "style='width:490px:margin-left:auto;margin-right:auto;'"; } ?> >
                  <? if($gfn){ echo "TICKET TO THE MOON "; } ?>
                  WEBSITE</button>
                        </form>
              </h6></td>
          </tr>
                                                ...
                                                if(data.length>0)
														{
															...
															if(data!='')
															{
																...
																{
																	// no driver coming
																	//alert ('cancelled 1'+data);
																	document.getElementById("confirmed").innerHTML="<b>Move marker to change pickup location</b>";
																	document.getElementById("confirmed").style.color="green";
																}
																else
																{
																$.each(data, function(key,value)
																	{
																		...
																		if(value.driver_id!='')
																			{
																				//alert('wet' + value.driver_id);
																				$.post('../../../MyDIO_functions/get_taxi_data.php', { driver_id: value.driver_id }, function(taxidata)
																					{
																						//alert('w' + taxidata.driver_fullname);
																						
																									var driver_name=taxidata.driver_fullname;
																									var pic_data = document.getElementById("profile_pic_dets").innerHTML;
																									//alert (pic_data);
																									if(pic_data==taxidata.profile_pic)
																									{
																										// Same profile no need to reload image 
																										//alert ('bingo');
																										}
																									else
																									{
																										//document.getElementById("toggle_loc_button").style.backgroundColor="#51b36d";
																										//document.getElementById("driver_status").setAttribute("value", 'false');
																										document.getElementById("profile_pic_dets").innerHTML=taxidata.profile_pic;
																										document.getElementById("confirmed").innerHTML="<table style='color:green;' width='295'><tr><td id='prof_pic' style='width:50px;background: url("+taxidata.profile_pic+");background-size:cover;'>&nbsp;</td><td><h4 align='center'><b>Driver coming!<p>"+driver_name+"</b></h4></td></tr></table>";
																										document.getElementById("confirmed").style.color="green";
																									}
    ...
    <div id="confirm_button" class="bottom_fixed_white">
              <div id="profile_pic_dets" style="display:none;" ></div>
        ...
        <tr>
                  <td colspan="2" id="nearest-driver" ><a class="change" style="color:#51b36d;"><b>Nearest driver</b></a></td>
                </tr>
        <tr>
                  <td colspan="2" id="waiting" ><a class="change" style="color:#51b36d;"><b></b></a></td>
                </tr>
        <tr>
...
<div class="to-top hidden"> <a id="toTop" href="../#">
  <div class="to-top-icon icon"></div>
  </a> </div>
